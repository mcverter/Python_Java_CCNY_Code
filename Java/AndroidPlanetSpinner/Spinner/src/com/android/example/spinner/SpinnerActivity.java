...
public class SpinnerActivity extends Activity {
	...
	protected Planet mSelectedPlanet;
	
	// records the number of planets
	protected int numPlanets;

	/**
	 * ArrayAdapter connects the spinner widget to array-based data.
	 * 
	 * We are using a specialized PlanetAdapter to  hold information 
	 * about the planets in the array.
	 */
	protected PlanetAdapter mLocalAdapter;
	Spinner spinner;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		String [] sizes = getResources().getStringArray(R.array.Sizes);
		...
		numPlanets = names.length;
		Planet [] planets = new Planet[numPlanets];
		int imageID, i;
		for (i = 0; i < numPlanets; i++)
		{
			imageID =  getResources().getIdentifier
					(names[i].toLowerCase(), "drawable", getPackageName()); 
			planets[i] = new Planet (names[i], moons[i], 
					distances[i], sizes[i], imageID);
		}
		
		/*
		 * Create a backing mLocalAdapter for the Spinner from a list of the
		 * planets using the array we have just created.
		 */

		mLocalAdapter = new PlanetAdapter (this,  
		...
		spinner.setAdapter(mLocalAdapter);
		...
		OnItemSelectedListener spinnerListener = new myOnItemSelectedListener(this,this.mLocalAdapter);
		spinner.setOnItemSelectedListener(spinnerListener);

		// Button added
		// when the button is clicked, it increments the spinner location by 1
		final Button button = (Button) findViewById(R.id.NextButton);
		button.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				mPos = (mPos + 1) % numPlanets;
	...
	public class myOnItemSelectedListener implements OnItemSelectedListener {
		...
		public myOnItemSelectedListener(Activity c, PlanetAdapter ad) {
		...
		public void onItemSelected(AdapterView<?> parent, View v, int pos, long row) {
			...
			SpinnerActivity.this.mSelectedPlanet = (Planet) parent.getItemAtPosition(pos);


			TextView resultText = (TextView)findViewById(R.id.SpinnerResult);
			resultText.setText(mSelectedPlanet.getName());

			TextView sizeText = (TextView) findViewById(R.id.SizeResult);
			sizeText.setText(mSelectedPlanet.getSize());

			TextView distanceText = (TextView) findViewById(R.id.DistanceResult);
			distanceText.setText(mSelectedPlanet.getDistance());

			TextView moonsText = (TextView) findViewById(R.id.MoonsResult);
			moonsText.setText(mSelectedPlanet.getMoons());

			ImageView planetImage = (ImageView) findViewById(R.id.PlanetImage);
			planetImage.setImageResource(mSelectedPlanet.getImageID());
		}
